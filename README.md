...
* **Optional**: A binary mask and pseudolabel annotations for guided merging (see [Guided Merging Documentation](docs/GUIDED_MERGING.md))
...
**New**: You can also provide pseudolabel annotations and a binary mask to merge ground-truth and pseudolabel data. The mask determines which regions use ground-truth (mask=1) vs pseudolabel (mask=0) annotations. See the [Guided Merging guide](docs/GUIDED_MERGING.md) for details.
